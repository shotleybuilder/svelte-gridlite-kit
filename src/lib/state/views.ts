...
export async function deleteView(
	...
	await db.query(
		`DELETE FROM _gridlite_column_state WHERE view_id = $1`,
		[viewId]
	);
	await db.query(
		`DELETE FROM _gridlite_views WHERE id = $1`,
		[viewId]
	);
}
...
export async function saveColumnState(
	db: PGlite,
	gridId: string,
	columns: { name: string; visible?: boolean; width?: number; position?: number }[],
...
export async function loadColumnState(
	...
	gridId: string,
	viewId: string = '__default__'
): Promise<{ name: string; visible: boolean; width: number | null; position: number | null }[]> {
