...
import type { ParameterizedQuery } from "@shotleybuilder/svelte-gridlite-kit/types";
...
export function createLiveQueryStore<T = Record<string, unknown>>(
  ...
  let liveUnsubscribe:
    | ((cb?: (results: LiveQueryResults<T>) => void) => Promise<void>)
    | null = null;
  let liveRefresh:
    | ((options?: { offset?: number; limit?: number }) => Promise<void>)
    | null = null;
  let destroyed = false;
  ...
  async function setupLiveQuery(sql: string, queryParams: unknown[]) {
    ...
    try {
      const liveQuery = await db.live.query<T>(
        sql,
        queryParams as any[],
        handleResults,
      );
