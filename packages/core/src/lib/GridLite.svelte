<script lang="ts">
	...
	/** Query adapter (e.g. createPGLiteAdapter({ db, table })) */
	...
	// Live query handle
	let store: LiveQueryHandle | null = null;
	...
	async function init() {
		try {
			// Initialize the adapter (runs migrations, introspects schema, etc.)
			await adapter.init();

			// Get columns from the adapter (populated for table mode, empty for query mode)
